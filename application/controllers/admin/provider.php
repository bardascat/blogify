...
class Provider extends CI_Controller {
	...
	function extensionsNew() {

		$sOutput = '';
		$aFiels = get_filenames('./assets/admin/resources/ext-3.4new/combine/');
		natcasesort($aFiels);
		foreach ($aFiels as $sFile) {
			$aFileData = pathinfo($sFile);
			...
			if ($aFileData["extension"] == "js") {
				$sOutput .= '/* ' . $sFile . "*/\n";
				$sOutput .= read_file('./assets/admin/resources/ext-3.4new/combine/' . $sFile) . "\n\n";
			}
		}

		$aFiels = get_filenames('./assets/admin/resources/ext-3.4new/customs/');
		natcasesort($aFiels);
		foreach ($aFiels as $sFile) {
			$sOutput .= '/* ' . $sFile . "*/\n";
			$sOutput .= read_file('./assets/admin/resources/js/customs/' . $sFile) . "\n\n";
